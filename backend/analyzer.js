...
export function cleanTitle(title, siteName = '') {
  ...
  return usableTitle(value) ? value.slice(0, 500) : '';
}
...
const NOISE = new Set('blog blogs article articles post posts news home page pages website official copyright reserved menu next previous share related minute minutes reading'.split(' '));
// Explicit, conservative keyword-only inflections. Phrases remain exact.
const KEYWORD_FORMS = new Map(Object.entries({ signatures: 'signature', documents: 'document', templates: 'template', businesses: 'business', tools: 'tool' }));

export function analyzeTitles(titles) {
  const distinct = new Map();
  for (const input of titles) {
    const title = cleanTitle(input);
    if (title && !distinct.has(titleKey(title))) distinct.set(titleKey(title), title);
  }
  titles = [...distinct.values()];
  const keywords = new Map();
  const patterns = new Map();
  titles.forEach((title, titleIndex) => {
    const normalized = matchingText(title);
    ...
    for (const word of new Set(words.filter(useful).map(word => KEYWORD_FORMS.get(word) || word))) keywords.set(word, (keywords.get(word) || 0) + 1);
    for (let start = 0; start < words.length; start++) {
      for (const size of [2, 3, 4]) {
        ...
        if (!pattern) {
          ...
          const display = title.normalize('NFKC').replace(/[’']/g, '').replace(/(?<=\p{L})[-‐‑](?=\p{L})/gu, ' ').replace(/[\p{Extended_Pictographic}\uFE0F\u200D]/gu, ' ').replace(/\s+/g, ' ').trim();
          pattern = { term, label: display.slice(tokens[start].index, end.index + end[0].length), words: chunk, matches: new Set(), starts: 0 };
  ...
  return { totalTitles: titles.length, phrases: phrases.slice(0, 10), keywords: rankedKeywords.slice(0, 10), longTails: longTails.slice(0, 10), rankedAnalysis };
}
